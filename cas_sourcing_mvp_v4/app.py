...
st.set_page_config(
    page_title="CAS Sourcing MVP v4",
    page_icon="🧪",
    layout="wide",
)

st.title("🧪 CAS Sourcing & Procurement Intelligence MVP v4")
st.caption(
    "CAS input → supplier discovery → visible pricing extraction → normalized price → bulk estimate → shortlist. "
    "v4 preserves prior modes and adds deeper product-link expansion plus safer CAS-confirmed extraction."
)
...
def render_price_and_bulk_sections(ranked: pd.DataFrame, desired_qty_g: float, desired_quantity: float, desired_unit: str) -> None:
    ...
    if visible.empty:
        st.warning(
            "No visible prices were extracted from the current data. The system can still provide supplier/source links, "
            "but the bulk estimate needs at least one visible price point."
        )
        return
...
if run_search:
    ...
    if data_mode == "Stable mock data":
        ...
        with st.spinner("Running live supplier discovery, product-link expansion, and CAS-confirmed extraction..."):
            raw_results, discovery_df = discover_live_suppliers(
    ...
    if data_mode == "Live supplier discovery":
        st.caption(
            "Live mode now expands supplier search pages into likely product-detail pages, then uses layered extraction with a CAS-confirmation safety gate. It may still miss prices hidden behind account logins, heavy JavaScript, or quote gates. "
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"cas_supplier_shortlist_v4_{cas_number.replace('-', '_')}.csv",
        mime="text/csv",
    )
else:
